...
/**
 *persistencia simple en JSON (un objeto JSON por linea).
 *no usa librerías externas para mantener compatibilidad.
 */

public class PersistenciaArch implements Serializable{
    ...
    public LoadedData loadAll() throws IOException {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                Double precio = getPrecio(line, "precio", 0.0);
                boolean enCola = getBooleanValue(line, "enCola", false);

                Vehiculo v = null;
                if ("AUTO".equalsIgnoreCase(tipo)) 
                {
                    v = new Automovil(marca, modelo, anio, color, carroceria, esUsado, kilometraje, mantenimiento, id, lavado, precio);
                } 
                else if ("CAMIONETA".equalsIgnoreCase(tipo)) 
                {
                    v = new Camioneta(marca, modelo, anio, color, carroceria, esUsado, kilometraje, mantenimiento, id, lavado, precio);
                } 
                else if ("MOTO".equalsIgnoreCase(tipo) || "MOTOCICLETA".equalsIgnoreCase(tipo)) 
                {
                    v = new Motocicleta(marca, modelo, anio, color, carroceria, esUsado, kilometraje, mantenimiento, id, lavado, precio);
                } 
                else 
                {
                    continue;
    ...
    private String vehiculoToJsonLine(Vehiculo v, boolean inStock, boolean enCola) 
    {
        ...
        sb.append("\"precio\":").append(String.format("%.0f",v.getPrecio())).append(",");
    ...
    private static double getPrecio(String jsonLine, String key, double defaultVal) 
    {
    String s = getStringValue(jsonLine, key);
    if (s == null || s.isEmpty()) {
